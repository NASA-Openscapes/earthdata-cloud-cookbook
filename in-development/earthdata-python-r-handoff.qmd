...
```{r}
## translate credentials from python to R
credentials_r <- py$credentials #YAY!

## pluck my information
mykey <- credentials_r %>% pluck("accessKeyId")
mysecretkey <- credentials_r %>% pluck("secretAccessKey")
mytoken <- credentials_r %>% pluck("sessionToken")

## translate s3 links from python to R
s3_links_r <- py$s3_links

## pluck and create my bucket (currently pasted from above but come back to do programatically)
# my_link <- pluck(s3_links_r, 1) # TODO stringr::str_split(my_link, "/").  
