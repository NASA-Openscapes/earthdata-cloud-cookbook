...
-   **Required packages:**
    ...
    -   `jsonlite`: change the format of returned content
...
```{r}
packages <- c('jsonlite','DT', 'rstac', 'devtools')
...
STAC Item represents data and metadata assets that are spatiotemporally coincident. Using our STAC Query, let's get the first item from our STAC collection search in the above cell.
