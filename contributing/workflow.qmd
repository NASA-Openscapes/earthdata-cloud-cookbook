...
### No TOC on a chapter landing page

*We're thinking through this convention*

We prefer not to have a table of contents (TOC) on every chapter page. Each chapter page should have short paragraph saying what chapter includes and "look at the navbar to see what’s here". 

Why? When someone contributes a new Tutorial by adding their notebook and pointing to it in the `_quarto.yml` file, it is immediately discoverable in the navbar. Having to add a link to that tutorial to a list on the [Tutorials chapter page](https://nasa-openscapes.github.io/earthdata-cloud-cookbook/tutorials/) is an extra step that a contributor might not do so it'll be less discoverable than if it only appeared in navbar.

Caveat: On the Tutorials page (2023-08-08) the brief text under each tutorial heading says what you'll find in that tutorial and this can't be conveyed in navbar headings. 
