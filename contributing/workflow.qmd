...
Any pull request, or PR, must be reviewed before it can be merged into
the Cookbook. If you know someone who is familiar with the content
you're proposing to add, request their review (TODO: add screenshot to
request reviewer).

The following notes come from a coworking session in which Amy Steiker
and Julie Lowndes reviewed [a pull
request](https://github.com/NASA-Openscapes/earthdata-cloud-cookbook/pull/171)
from Andy Barrett.
([Video](https://drive.google.com/file/d/1LG82x2csIKqNx7KewkkLlMfFFKl1ioRW/),
33 min)

Generic tips on reviewing a pull request (video w Stef & Julie)

From the pull request page in GitHub browser, look at the elements of
the pull request.

![](images/Screenshot%202023-07-10%20at%2012.55.55%20PM.png)
...
-   Checks tab (usually we skip over it!):
    -   Shows status of the Github Action that renders and deploys the
        Cookbook. We can know whether this PR has the ability to be
        deployed
    -   This means two checks have passed

Now, switching to the Hub:

-   Terminal:
    -   Switch over to the Main branch and pull so that we have the most
        recent changes from remote.
    -   Git checkout to the branch that has the PR: reorg_how_to_guides
        (note, this PR has been merged and the branch deleted so you
        won't see this one; apply this example to your own)
    -   Quarto preview - this will build the book with Andy's suggested
        edits.
...
-   Now we see a minor issue with capitalization in the
    ...
    -   In GitHub, we can add a suggested edit by clicking the "plus"
        button below the line in question
        -   TODO: add some mechanics of making suggested commits rather
            than comments to speed the workflow.
...
We will review each individual `.md` file and can come back to the
main`_quarto.yml` if we see an issue with the main navigation.
...
including the number of changes requested. Some in the middle of a long
list of edits can be marked as hidden conversations, so this can be
helpful to the submitter to know they've seen everything.
